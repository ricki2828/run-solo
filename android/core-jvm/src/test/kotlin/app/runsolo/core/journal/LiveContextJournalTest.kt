...
class LiveContextJournalTest {
    ...
    fun `live context and cue fired lines round trip`() {
        val lines = listOf(
            ...
            JournalLine.CueFired(t0 + 900_000, w0 + 900_000, JournalLine.FiredKind.compare, "be:5000", 3, 880_500),
    ...
    fun `a full context stays a few KB`() {
        val full = context.copy(
            boards = List(3) { b ->
                LiveBoard(
                    key = "be:10000-$b", label = "10K", kind = LiveBoardKind.distance, targetM = 10_000.0,
    ...
    fun `replay keeps the context and fired cues aside, the run timeline is unchanged`() {
        ...
        val withLive = JournalReplay.read(
            enc(
                ...
                JournalLine.LiveContextLine(t0, w0, context),
                sample(t0 + 1000),
                JournalLine.CueFired(t0 + 1000, w0 + 1000, JournalLine.FiredKind.compare, "be:5000", 1, 1000),
        ...
        assertEquals(listOf("be:5000"), withLive.cuesFired.map { it.key })
    }
    ...
    fun `contract limits are enforced`() {
        ...
        val curve = context.cooperCurve!!
        // Must increase, end at F(12) = 1, be finite.
        assertFailsWith<IllegalArgumentException> { context.copy(cooperCurve = curve.reversed()) }
        assertFailsWith<IllegalArgumentException> { context.copy(cooperCurve = curve.dropLast(1) + 0.99) }
        assertFailsWith<IllegalArgumentException> { context.copy(cooperCurve = listOf(Double.NaN) + curve.drop(1)) }
        assertFailsWith<IllegalArgumentException> { context.copy(cooperHistory = listOf(Double.POSITIVE_INFINITY)) }
        assertFailsWith<IllegalArgumentException> { b.entries.first().copy(finalMetric = Double.NaN) }
        assertFailsWith<IllegalArgumentException> { LiveEntry("x", w0, liveRepPacesSecPerKm = listOf(Double.NaN), finalMetric = 1.0) }
    }
